...
impl DiskManager {
    ...
    pub fn read_disk_sync_all(&mut self, path: &str) -> StorageResult<(usize, Bytes)> {
    ...
    pub fn read_disk_sync(&mut self, path: &str, start_pos: u64, bytes_to_read: usize) -> StorageResult<(usize, Bytes)> {
        let mut file = File::open(path)?;
    ...
    // TODO: add a statistics member(from `DiskManager`) with lifetime in `DiskReadSyncIterator` to record?
        ...
        T: StorageReaderIterator,
    {
        if Path::new(disk_path).exists() {
            return Err(io::Error::new(io::ErrorKind::AlreadyExists, "disk file to write already exists").into());
        }
...
mod tests {
    ...
    fn test_simple_write_read_sync() {
        ...
        disk_manager.write_disk_sync_all(path, content.as_bytes()).expect("write_disk_sync_all failed");
        ...
        let (bytes_read, bytes) = disk_manager.read_disk_sync_all(path).expect("read_disk_sync_all failed");
        assert_eq!(bytes_read, 2 * content.len());
        assert_eq!(bytes, Bytes::from(content.to_owned() + content));

        let (bytes_read, bytes) = disk_manager.read_disk_sync(path, content.len() as u64, content.len()).expect("read_disk_sync_all failed");
    ...
    fn test_iterator_read() {
        ...
        disk_manager.write_disk_sync_all(path, content.as_bytes()).expect("write_disk_sync_all failed");
        let mut iterator = disk_manager.disk_read_sync_iterator(path, 2).expect("disk_read_sync_iterator failed");
        let mut start_pos = 0;
        loop {
            if start_pos >= content.len() {
                break;
            }
            let bytes_read = iterator.next().expect("iterator early ended").expect("iterator read failed");
            let buffer = iterator.buffer();
            assert_eq!(&content.as_bytes()[start_pos..start_pos + bytes_read], &buffer[..bytes_read]);
    ...
    fn test_write_reader_to_disk_sync() {
        ...
        let content = "bhjoilkmnkbhaoijsdklmnjkbhiauosdjikbhjoilkmnkbhaoijsdklmnjkbhiauosdjik";
        disk_manager.write_disk_sync_all(path, content.as_bytes()).expect("write_disk_sync_all failed");
        let mut iterator = disk_manager.disk_read_sync_iterator(path, 1).expect("disk_read_sync_iterator failed");
        let output_path = "test_disk_manager3_output.txt";
        disk_manager.write_reader_to_disk_sync::<DiskReadSyncIterator>(iterator, output_path).expect("write_reader_to_disk_sync failed");

        let (bytes_read, bytes) = disk_manager.read_disk_sync_all(output_path).expect("read_disk_sync_all failed");
        assert_eq!(bytes_read, content.len());
        assert_eq!(bytes, Bytes::from(content));
        let file_size = disk_manager.file_size(output_path).expect("file_size failed");
        assert_eq!(file_size, content.len() as u64);

        disk_manager.remove_file(path).expect("remove_file failed");
        assert!(!Path::new(path).exists());
        disk_manager.remove_file(output_path).expect("remove_file failed");
